...
void XBoard_Loop(S_BOARD *pos, S_SEARCHINFO *info) {
    ...
    int depth = -1, movestogo[2] = {30, 30}, movetime = -1; //Sus
    int time = -1, inc = 0;
    int engineSide = BOTH;
    int timeLeft;
    int mps;
    ...
    while (TRUE) {
        ...
        if (!fgets(inBuf, 60, stdin)) continue;
        ...
        if (!strcmp(command, "level")) {
            ...
            time = -1;
            printf("DEBUG level timeLeft:%d movesToGo:%d mps%d\n", timeLeft, movestogo[0], inc, mps);
            continue;
        }
        ...
        if (!strcmp(command, "setboard")) {
            engineSide = BOTH;
            Parse_FEN(inBuf+9, pos); continue;
        }
