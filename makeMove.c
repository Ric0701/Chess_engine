...
#define HASH_PCE(oce, sq)   (pos -> posKey ^= (PieceKeys[(pce)][(sq)]))
...
static void ClearPiece(const int sq, S_BOARD *pos) {
    ...
    //Remove pieces from the piecelist
...
int MakeMove(S_BOARD *pos, int move) {
    ...
    int promoPce = PROMOTED(move);
    if (promoPce != EMPTY) {
        ASSERT(PieceValid(promoPce) && !PiecePawn[promoPce]);
        ClearPiece(to, pos);
        AddPiece(to, pos, promoPce);
    }
    ...
    if (SqAttacked(pos -> KingSq[side], pos -> side, pos)) { // Soon to be removed in future
        TakeMove(pos);
        return FALSE;
    }
...
void TakeMove(S_BOARD *pos) {
    ...
    if (PieceKing[pos -> pieces[from]]) { // Soon to be removed in future
